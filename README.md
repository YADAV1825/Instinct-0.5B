...
| **Dataset** | [The Pile by EleutherAI](https://pile.eleuther.ai/) |
| **Tokenizer** | Pythia Tokenizer |
| **Tokens Seen** | 150 Billion |
...
* Dataset streaming from **The PILE by EleutherAI**.
...
class GPT(nn.Module):
    def __call__(self, input_ids):
        ...
        mask = nn.attention.make_causal_mask(jnp.ones((batch, seq_len), dtype=jnp.bool_))
        x = nn.Embed(
            VOCAB_SIZE, D_MODEL,
            embedding_init=nn.initializers.normal(0.02), dtype=jnp.bfloat16,
        )(input_ids)
        RematBlock = nn.remat(Block)
        for _ in range(N_LAYERS):
            x = RematBlock()(x, mask)
        x = RMSNorm(D_MODEL)(x)
        return nn.Dense(VOCAB_SIZE, use_bias=False, dtype=jnp.bfloat16)(x)
